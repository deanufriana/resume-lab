  ...
  <div class="max-w-2xl mx-auto animate-in fade-in slide-in-from-bottom-4 duration-500">
    <Card class="border-dashed border-2 bg-card/50 backdrop-blur-sm">
      <CardHeader class="text-center">
        <div class="mx-auto size-12 rounded-full bg-primary/10 flex items-center justify-center mb-4">
          <FileUp class="size-6 text-primary" />
        </div>
      ...
      <CardContent class="flex flex-col gap-6">
        ...
        <div class="flex items-center justify-center gap-4 pt-4">
          <Button variant="outline" @click="emit('download-example')">
            <Download data-icon="inline-start" /> Example JSON
          </Button>
          <Button v-if="resumeData.basics?.name" @click="emit('view-current')">
            View Current <Eye data-icon="inline-end" />
          </Button>
        </div>
      </CardContent>
    </Card>
