  ...
  <div ref="previewContainer" id="resume-preview" class="relative bg-white text-gray-900 font-sans p-10 min-h-[1100px]">
    ...
    <header class="border-b-4 border-gray-900 pb-8 mb-8">
      <div class="flex flex-col items-center text-center">
        <h1 class="text-4xl font-extrabold tracking-widest font-outfit uppercase mb-2">
          {{ resumeData.basics?.name }}
        </h1>
        <p class="text-lg font-bold text-gray-600 uppercase tracking-widest font-outfit mb-4">
        ...
        <div class="flex flex-wrap justify-center gap-x-6 gap-y-2 text-sm font-medium">
          <div v-if="resumeData.basics?.email" class="flex items-center gap-1">
            <span class="text-gray-400 font-bold">E:</span> {{ resumeData.basics.email }}
          </div>
          <div v-if="resumeData.basics?.phone" class="flex items-center gap-1">
            <span class="text-gray-400 font-bold">P:</span> {{ resumeData.basics.phone }}
          </div>
          <div v-if="resumeData.basics?.location" class="flex items-center gap-1">
          ...
          <div v-if="resumeData.basics?.url" class="flex items-center gap-1">
            ...
            <a :href="resumeData.basics.url" class="hover:text-blue-600 transition-colors" target="_blank">{{ resumeData.basics.url.replace(/^https?:\/\//, '') }}</a>
          </div>
        </div>
      </div>
    </header>
      ...
      <div class="space-y-8">
        <div v-for="(work, index) in resumeData.work" :key="index">
          <div class="flex justify-between items-baseline mb-2">
            <div>
              <h3 class="text-lg font-extrabold uppercase tracking-tight">{{ work.position }}</h3>
              <h4 class="text-base font-bold text-gray-600">{{ work.name }}</h4>
            </div>
            <div class="text-sm font-black text-gray-500 uppercase tracking-tighter">
              {{ formatDate(work.startDate) }} — {{ formatDate(work.endDate) || 'Present' }}
            </div>
          </div>
          <p v-if="work.summary" class="text-gray-700 font-medium mb-3">{{ work.summary }}</p>
          <ul v-if="work.highlights" class="resume-list grid grid-cols-1 gap-2">
            <li v-for="(highlight, hIndex) in work.highlights" :key="hIndex" class="text-sm text-gray-800 leading-snug">
              {{ highlight }}
            </li>
          </ul>
        </div>
        ...
        <div class="space-y-6">
          <div v-for="(edu, index) in resumeData.education" :key="index">
            <div class="flex justify-between items-start gap-2">
              <h3 class="font-extrabold text-sm uppercase leading-tight">{{ edu.studyType }} <br/> {{ edu.area }}</h3>
              <span class="text-[10px] font-black text-gray-500 whitespace-nowrap">{{ formatDate(edu.startDate) }} - {{ formatDate(edu.endDate) }}</span>
            </div>
            <p class="text-sm font-bold text-gray-600">{{ edu.institution }}</p>
            <p v-if="edu.score" class="text-xs font-medium text-gray-500 mt-1 italic">Honors / GPA: {{ edu.score }}</p>
          </div>
        </div>
      </section>
    </div>
    ...
    <div v-if="resumeData.awards?.length || resumeData.certificates?.length" class="mt-10 pt-8 border-t-2 border-gray-100 grid grid-cols-1 md:grid-cols-2 gap-10">
      <section v-if="resumeData.awards?.length">
        ...
        <ul class="space-y-2">
          <li v-for="(award, index) in resumeData.awards" :key="index" class="text-sm">
            <span class="font-black">{{ award.title }}</span> — <span class="text-gray-600 font-bold">{{ award.awarder }}, {{ formatDate(award.date) }}</span>
          </li>
        </ul>
      </section>
      ...
      <section v-if="resumeData.certificates?.length">
        ...
        <ul class="space-y-2 text-sm">
          <li v-for="(cert, index) in resumeData.certificates" :key="index">
            <span class="font-black">{{ cert.name }}</span> — <span class="text-gray-600 font-bold">{{ cert.issuer }}</span>
          </li>
        </ul>
      </section>
    </div>
